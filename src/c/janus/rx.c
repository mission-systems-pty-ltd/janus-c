...
//  exit(1);
  ...
  if (rx->state == STATE_32_CHIP_PREAMBLE)
  {
    ...
    // fprintf(stderr, "skip size %f\n", skip_size);

    // fprintf(stderr, "simple_rx->rx->chip_size %f\n", rx->chip_size);
    // fprintf(stderr, "bband_time_counter %f\n", rx->bband_time_counter);
    // fprintf(stderr, "simple_rx->rx->bband_cfar_init_counte %f\n", rx->bband_cfar_init_counter);
    // fprintf(stderr, "chips_alignment_offset %f\n", rx->chips_alignment_offset);
